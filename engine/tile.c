...
void putTile(SDL_Renderer *pRenderer, SDL_Texture *pTex, Uint16 _x,Uint16 _y, Uint16 _index, Uint16 tile_size, Uint16 tile_set_width, Uint16 zoom) {
    SDL_Rect _tmpDstRt;
...
void drawWorld(SDL_Renderer *pRender, SDL_Texture *pTileSet, int tile_size, int tileset_width, int zoom, int x, int y, int map_size, Sint16 *map) {
    for (int i=0; i<map_size*map_size; i++) {
        Sint16 _index = map[i];

        if (_index != -1) {
            putTile(pRender, pTileSet, i%map_size, i/map_size, _index, tile_size, tileset_width, zoom);
        }
    }
}
...
SDL_bool loadMap(const char *filename, Sint16 *map[2]) {
    ...
    SDL_RWread(rw, map[0], sizeof(Uint16), 225);
    SDL_RWread(rw, map[1], sizeof(Uint16), 225);
    SDL_RWclose(rw);
...
SDL_bool saveMap(const char *filename, Sint16 *map[2]) {
    ...
    SDL_RWwrite(rw, map[0], sizeof(Uint16), 225);
    SDL_RWwrite(rw, map[1], sizeof(Uint16), 225);
    SDL_RWclose(rw);
